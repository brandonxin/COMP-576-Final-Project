...
# Constants for string slicing
BASE_LEN = 89  # Adjust based on your filenames
END_IMG_LEN = 4  # Length of ".tif" or other image extensions
END_MASK_LEN = 9  # Length of "_mask.tif" or other mask extensions

DATA_PATH = "D:\\kaggle\\input\\lgg-mri-segmentation\\kaggle_3m\\"
...
for sub_dir_path in glob.glob(DATA_PATH + "*"):
    if os.path.isdir(sub_dir_path):
        dirname = os.path.basename(sub_dir_path)  # Correctly extract folder name for both Windows/Linux
        for filename in os.listdir(sub_dir_path):
            image_path = os.path.join(sub_dir_path, filename)  # Correct path joining
...
# path = 'D:\\kaggle\\input\\lgg-mri-segmentation\\kaggle_3m\\TCGA_CS_4941_19960909\\TCGA_CS_4941_19960909_1.tif'
# numeric_part = path[BASE_LEN:-END_IMG_LEN]
# print(numeric_part)  # 输出应该是 '1'
# mask_path = 'D:\\kaggle\\input\\lgg-mri-segmentation\\kaggle_3m\\TCGA_CS_4941_19960909\\TCGA_CS_4941_19960909_1_mask.tif'
# numeric_part_mask = mask_path[BASE_LEN:-END_MASK_LEN]
# print(numeric_part_mask)  # 输出应该是 '1'
...
print(df.head())
print(df.shape)  # 检查 DataFrame 的行数和列数
...
for i, value in enumerate(df.diagnosis.value_counts().values):
    ax.text(
        i, value + 50,  # 标注位置（x: 柱的索引, y: 柱的高度 + 偏移量）
        str(value),  # 标注文本
        ha='center', va='bottom',  # 水平、垂直对齐方式
        fontsize=12, fontweight="bold", color="black"  # 字体样式
    )

# 显示总图像数量
ax.text(
    1.2, max(df.diagnosis.value_counts().values) + 200,  # 标注位置
    f"Total {len(df)} images",  # 文本内容
    size=15, color="black",
    ha="center", va="center",
...
grid[0].set_title("Positive", fontsize=8)
grid[0].axis("off")
grid[1].imshow(sample_no_arr)
grid[1].set_title("Negative", fontsize=8)
grid[1].axis("off")

grid[2].imshow(sample_yes_arr[:,:,0], cmap="hot")
grid[2].set_title("Positive", fontsize=8)
grid[2].axis("off")
grid[3].imshow(sample_no_arr[:,:,0], cmap="hot")
grid[3].set_title("Negative", fontsize=8)
grid[3].axis("off")#set_title("No", fontsize=15)

# annotations
plt.figtext(0.36,0.91,"Original", va="center", ha="center", size=10)
plt.figtext(0.66,0.91,"With hot colormap", va="center", ha="center", size=10)
plt.suptitle("LGG Segmentation Dataset", y=.95, fontsize=20, weight="bold")
    ...
    for idx in range(len(inputs)):
        ...
        if image is True:           
            ...
            if img.ndim == 3:
                ...
                raise ValueError("Unexpected dimensions in image.")
...
# Segmentation Quality Metric Coefficient
#[26]
...
def train_model(model_name, model, train_loader, val_loader, train_loss, optimizer, lr_scheduler, num_epochs):  
    ...
    for epoch in range(num_epochs):
        ...
        for i_step, (data, target) in enumerate(train_loader):
            ...
            if target.ndim == 3:  # 如果目标是 [batch_size, height, width]
                target = target.unsqueeze(1)  # 变为 [batch_size, 1, height, width]
...
#[32]Change Model History Records
...
# [36]Plates of images of predictive overlapping
# DATA
